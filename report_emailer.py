...
Defaults can be provided via environment variables:
  REPORTS_DIR, SENT_DIR, FAILED_DIR, POLL_INTERVAL_SECONDS, MIN_REPORT_AGE_SECONDS,
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--min-age",
        type=int,
        default=int(_env_or_default("MIN_REPORT_AGE_SECONDS", "5")),
        help="Minimum age (in seconds) a report must be before sending (default: 5).",
    )
...
def _is_report_ready(report_path: Path, min_age_seconds: int) -> bool:
    try:
        last_modified = report_path.stat().st_mtime
    except FileNotFoundError:
        return False

    return (time.time() - last_modified) >= min_age_seconds
    ...
    try:
        while True:
            ...
            for report_path in report_files:
                if not _is_report_ready(report_path, args.min_age):
                    continue
                try:
                    _send_report(
